...
#   Step 1 baseline: Results/generated/baseline/sparse_variant_{a,b,c}.s
#
...
OUT="$ROOT/Results/generated/patched"
TESTDIR="$ROOT/tests"
BASE="$ROOT/Results/generated/baseline"
...
for v in a b c; do
  if [[ ! -f "$BASE/sparse_variant_${v}.s" ]]; then
    ...
    echo "Run ./Scripts/step1_baseline.sh first (with LLVM_UNPATCHED_LLC)." >&2
    exit 1
  fi
done
