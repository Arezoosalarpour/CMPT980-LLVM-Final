...
`summary.json` under `near_misses` — **not** implementation candidates.
...
| Path | Description |
|------|-------------|
| `results.json` | Per-mask results |
| `summary.json` | Aggregates + near_misses |
| `work/` | Per-case `.ll` / `.s` |
| `commands.log` | Reproduce log |
| `llvm_lowering.patch.save` | Project patch snapshot from this run |
