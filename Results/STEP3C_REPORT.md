...
Lit test diff: `vector-shuffle-combining-avx2.ll.patch`
...
Log: `broader_runs.log`

### Semantic verification (A/B/C)

```bash
./Scripts/step3c_finalize.sh   # semantic section
# or manually:
LLC=~/llvm-project/build-x86/bin/llc
for v in a b c; do
...
Result: **PASS** — see `semantic/verify.log`

### 71-function comparison

```bash
python3 Scripts/step3c_compare.py Results/generated/step3c
```

Result: **1 changed** (`combine_and_pshufb`), **70 unchanged**. Data: `step3c_compare.json`
...
Full snippet: `asm/combine_and_pshufb.s`
...
Backend patch artifact: `X86ISelLowering.patch` (346 lines)
